...
private fun handleMockData() {
    val responseMatchedMemberList: ResponseData = getMockMatchedMemberList()
    updatePage(Page(responseMatchedMemberList.data.totalPages, responseMatchedMemberList.data.totalElements, responseMatchedMemberList.data.currentPage, responseMatchedMemberList.data.pageSize))
    addMatchingResult(responseMatchedMemberList.data.content)
}

private fun handleUserMockData() {
    val responseUserInfo: UserResponse = getMockUserInfo()
    updateUserInfo(responseUserInfo.data.member)
}

private fun getMockMatchedMemberList(): ResponseData {
    return ResponseData(
        200,
        "OK",
        "추천 룸메이트 조회 성공",
...
private fun getMockUserInfo(): UserResponse {
    return UserResponse(
        200,
        "No Ok",
        "더미 데이터 불러오기",
